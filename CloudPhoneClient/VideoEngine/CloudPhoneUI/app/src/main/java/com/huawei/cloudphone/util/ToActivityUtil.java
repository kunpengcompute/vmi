...
public class ToActivityUtil {
    ...
    public static void toFullScreenActivity(Activity activity, VideoConf videoConf, int reqCode) {
        // opus编码比特率
        videoConf.setBitrate(SPUtil.getInt(SPUtil.MIC_BITRATE, 192000));
        // 增加编码参数
        ...
        videoConf.setVideoFrameRate(SPUtil.getInt(SPUtil.VIDEO_FRAME_RATE, 30));// 暂不使用
        videoConf.setVideoForceLandscape(SPUtil.getBoolean(SPUtil.VIDEO_FORCE_LANDSCAPE, false));// 暂不使用
        videoConf.setVideoFrameSizeWidth(SPUtil.getInt(SPUtil.VIDEO_FRAME_SIZE_WIDTH, Constant.VMI_720P_WIDTH));
        videoConf.setVideoFrameSizeHeight(SPUtil.getInt(SPUtil.VIDEO_FRAME_SIZE_HEIGHT, Constant.VMI_720P_HEIGHT));
        videoConf.setVideoFrameSizeWidthAligned(SPUtil.getInt(SPUtil.VIDEO_FRAME_SIZE_WIDTH_ALIGNED, Constant.VMI_720P_WIDTH));
        videoConf.setVideoFrameSizeHeightAligned(SPUtil.getInt(SPUtil.VIDEO_FRAME_SIZE_HEIGHT_ALIGNED, Constant.VMI_720P_HEIGHT));
        videoConf.setVideoRenderOptimize(SPUtil.getBoolean(SPUtil.VIDEO_RENDER_OPTIMIZE, true));
        ...
        videoConf.setVideoForceKeyFrame(SPUtil.getInt(SPUtil.VIDEO_FORCE_KEY_FRAME, 0));
        videoConf.setVideoInterpolation(SPUtil.getInt(SPUtil.VIDEO_INTERPOLATION, 0));
        videoConf.setCrf(SPUtil.getInt(SPUtil.VIDEO_CRF, 34));
        videoConf.setMaxCrfRate(SPUtil.getInt(SPUtil.VIDEO_MAX_CRF_RATE, 20000000));
        videoConf.setVbvBufferSize(SPUtil.getInt(SPUtil.VIDEO_VBV_BUFFER_SIZE, 1000));
        videoConf.setStreamWidth(SPUtil.getInt(SPUtil.VIDEO_STREAM_WIDTH, Constant.VMI_720P_WIDTH));
        videoConf.setStreamHeight(SPUtil.getInt(SPUtil.VIDEO_STREAM_HEIGHT, Constant.VMI_720P_HEIGHT));
