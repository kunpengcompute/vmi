...
so_lists=(
    ${cur_file_path}/libs/arm64-v8a/*
    ${cur_file_path}/Decoder/libs/arm64-v8a/*
)

include_lists=(
    ${cur_file_path}/Control/VideoEngineClient.h
)
...
build()
{
    ...
    if [ -n "${MODULE_OUTPUT_DIR}" ];then
        mkdir -p ${MODULE_OUTPUT_DIR}/include
        for h_name in ${include_lists[@]}
        do
            info "cp ${h_name} ${MODULE_OUTPUT_DIR}/include/*}"
            cp ${h_name} ${MODULE_OUTPUT_DIR}/include/
        done
        for so_name in ${so_lists[@]}
        do
            if [ "${so_name}" != "${cur_file_path}/libs/arm64-v8a/libc++_shared.so" ] \
            && [ "${so_name}" != "${cur_file_path}/Decoder/libs/arm64-v8a/libc++_shared.so" ]; then
