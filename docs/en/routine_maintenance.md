...
<!-- md-trans-meta sourceCommit=377087a2bd241cd72440e17b86df0cd93ee57185 translatedAt=2026-09-08T02:31:33.054Z pushedAt=2026-09-15T12:27:38.218Z -->
...
Generate a report based on the inspection operations, as shown in [Video Stream Cloud Phone Inspection Report](#video-stream-cloud-phone-inspection-report).

After a video stream cloud phone is deployed for the first time, you are advised to perform an inspection immediately.

#### Video Stream Cloud Phone Inspection Report
...
### Checking the Operating Environment Status
...
3. If the service status is abnormal, run the following command to check the cause of the Docker service startup failure:
    ...
    If the fault persists, restart the Docker service (see step 4) or contact Huawei technical support.
...
### Checking the Video Stream Cloud Phone Status
...
4. Check the process status.
    ...
    - Check whether the `VmiAgent` process exists. If the process is not found, it indicates that `VmiAgent` has been terminated and the video stream cloud phone cannot work normally. In this case, contact Huawei technical support.
...
3. Check the system memory usage of the video stream cloud phone server.
...
The system load indicates the current CPU workload, which measures the average number of threads in the running queue during a specific time interval. If the load is too high, the server cannot process other requests and operations, or even breaks down.
...
### Basic Cloud Phone Maintenance Tool
...
### Querying Video Stream Cloud Phone Logs

#### Querying Logs on the Android Device

##### Application Stack Information During ANR
...
#### Querying Docker Container Logs
...
4. Run the `inspect` command provided by Docker to view container details. In the following command, ${android_id} indicates the ID of the started instance.
    ...
    ```bash
    docker inspect --format='{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' android_${android_id}
    ```
...
For details about risky operations on server hardware, see [Risky Operations on Hardware](#risky-operations-on-hardware). For details about risky operations on software, see [Risky Operations on Software](#risky-operations-on-software).

#### Risky Operations on Hardware
...
You need to perform routine maintenance on project operations and output O&M reports. For details, see [Video Stream Cloud Phone Routine Maintenance Report](#video-stream-cloud-phone-routine-maintenance-report).

After a video stream cloud phone is deployed for the first time, you are advised to perform an inspection immediately.

### Video Stream Cloud Phone Routine Maintenance Report
