...
<!-- md-trans-meta sourceCommit=536398a299a46f062322e447e50a05175f394800 translatedAt=2026-09-08T02:29:07.320Z pushedAt=2026-09-18T09:33:42.168Z -->

## Software Deployment<a name="ZH-CN_TOPIC_0000002518386576"></a>

### Environment Requirements<a name="ZH-CN_TOPIC_0000002518226658"></a>

#### Hardware Environment<a name="ZH-CN_TOPIC_0000002518226662"></a>
...
|Item|Hardware Configuration Scheme 1|Hardware Configuration Scheme 2|Hardware Configuration Scheme 3|Hardware Configuration Scheme 4|Hardware Configuration Scheme 5|
|--|--|--|--|--|--|
...
|OS|openEuler 24.03 LTS SP1|openEuler 24.03 LTS SP1|openEuler 24.03 LTS SP1|openEuler 24.03 LTS SP1|openEuler 24.03 LTS SP1|
|System/Kernel version|6.6.0-72.0.0|6.6.0-72.0.0|6.6.0-72.0.0|6.6.0-72.0.0|6.6.0-72.0.0|
...
#### Software Environment<a name="ZH-CN_TOPIC_0000002549866439"></a>
...
##### Video Stream Engine<a name="ZH-CN_TOPIC_0000002549746419" id="video-stream-engine"></a>
...
|No.|Software Package|Description|How to Obtain|Configuration Scheme 1|Configuration Scheme 2|Configuration Scheme 3|Configuration Scheme 4|Configuration Scheme 5|
|--|--|--|--|--|--|--|--|--|
...
>Before using the software package, read [Kunpeng BoostKit User License Agreement 2.0](https://www.hikunpeng.com/en/legal/developer/boostkit/software/protocol). If you choose to continue using the package, you are deemed to have agreed to the terms and conditions of the agreement.

### Deploying a Cloud Phone<a name="ZH-CN_TOPIC_0000002549746425"></a>

#### Environment Configuration<a name="ZH-CN_TOPIC_0000002518386582"></a>

##### Configuring the BIOS<a name="ZH-CN_TOPIC_0000002518226660"></a>
...
##### (Configuration Scheme 1, Optional) Upgrading the NVMe Firmware<a name="ZH-CN_TOPIC_0000002518226644"></a>
...
##### (Configuration Scheme 1) Configuring the GPU Working Mode<a name="ZH-CN_TOPIC_0000002518226676"></a>
...
##### Deploying the Basic Environment for the Kbox Container<a name="ZH-CN_TOPIC_0000002549746421" id="deploying-the-basic-environment-for-the-kbox-container"></a>

For details about how to deploy a Kbox cloud phone container, see [Kbox Cloud Phone Container Installation Guide](https://www.hikunpeng.com/document/detail/en/kunpengcps/boostcph/kboxcpc_ad15/docs/en/install_guide.md).

##### (Optional) Deploying the Containerd Environment<a name="ZH-CN_TOPIC_0000002549866409" id="deploying-the-containerd-environment"></a>
...
| CNI Plugin | v1.4.1 | The Container Network Interface (CNI) is a specification and a set of libraries for configuring network interfaces in Linux containers. | [Link](https://github.com/containernetworking/plugins/releases/download/v1.4.1/cni-plugins-linux-arm64-v1.4.1.tgz) |
...
#### (Configuration Scheme 2/3/4/5) Installing the GPU Driver<a name="ZH-CN_TOPIC_0000002549866411"></a>

You need to install the GPU driver each time the server is restarted if you use hardware configuration scheme 2/3/4/5.

1. Obtain `VAGPU-A15-C-F-26.02.06.00.RC2.tgz` according to [Software Environment](https://www.hikunpeng.com/document/detail/en/kunpengcps/boostcph/kboxcpc_ad15/docs/en/install_guide.md#d22-software-environment), upload it to the `~/dependency/` directory, and decompress it to obtain the kernel-space GPU driver.
...
2. Copy the firmware in the driver package to the `/lib/firmware/` directory.
...
3. Install the GPU driver.
    ...
    Taking DC1000/DC1000C as an example, the following core binding methods are for reference only. Adjust them based on actual circumstances.
    ...
    Configuration scheme 3 (new Kunpeng 920 processor model + 8 x DaoCloud DC1000/DC1000C)
    ...
    Configuration scheme 5 (new Kunpeng 920 processor model + 8 x DaoCloud DC1000):
...
4. Wait until the script execution is complete and check the kernel logs.

    ```bash
    dmesg | grep VAGPU | grep version
    ```

    In the command output, if the kernel-space GPU driver version is consistent with the GPU firmware version, the GPU driver is installed successfully.
...
>3. The new DaoCloud driver version removes `va_gfx.ko` and `va_pci.ko` and merges them into `va_gpu.ko`. To uninstall this version, only `va_gpu.ko` needs to be removed.
>
>
> ```bash
> rmmod va_gpu
> ```
>
> The commands for uninstalling the old driver version are as follows:
>
> ```bash
> rmmod va_gfx
> rmmod va_pci 
...
#### Creating an Image<a name="ZH-CN_TOPIC_0000002549866429" id="creating-an-image"></a>

##### Creating a Kbox Image<a name="ZH-CN_TOPIC_0000002549866417"></a>
...
2. Decompress the `Kbox-patches-AOSP15.zip` package, extract the `android_kbox_aosp15.sh`, `base_box_aosp15.sh`, and `hardware_bind.cfg` files from `deploy_scripts` to the`/home/kbox_video` directory, and grant permissions on the files. Ensure that the file owner has the read, write, and execute permissions while users in the owner group and others have only the read and execute permissions.
...
3. Create a Kbox image named `kbox:origin`.
    ...
    4. (Hardware configuration scheme 2/3/4/5) If hardware configuration scheme 2/3/4/5 is used, decompress the GPU driver package `VAGPU-A15-C-F-26.02.06.00.RC2.tgz` and upload the extracted files to the `~/dependency/deploy_scripts` directory on the server.
...
##### Creating a Video Stream Cloud Phone Image<a name="ZH-CN_TOPIC_0000002518386578"></a>
...
1. Obtain `CloudPhoneApk.tar.gz`, `DemoVideoEngine.tar.gz`, and `BoostKit-boostcph-videoengine_*_15.zip` based on [Video Stream Engine](#video-stream-engine) and upload them to the `/home/kbox_video` directory on the server.
...
2. Create a video stream cloud phone image. You can use the default image name or specify an image name.
    ...
    - If a customized image name is used, run the following command. Specify the image names of the Kbox basic cloud phone and video stream cloud phone in the format of *{image_name}:{tag}*. In the following command, `kbox` and `video` are image names, and `origin` and `latest` are tags.
        ...
        >![](public_sys-resources/icon-note.gif) **NOTE**
        >
        >The image name can contain only digits and lowercase letters, and must start with a lowercase letter. The tag can contain only digits and letters. If the image name of the video stream cloud phone is changed, refer to [Creating a Base Data Volume](#creating-a-base-data-volume) and change the image name in the `cfct_config` file to the customized image name.
...
#### Setting cfct_config and hardware_bind.cfg (Configuration Scheme 1)<a name="ZH-CN_TOPIC_0000002518386584"></a>
...
3. The nodes of the NETINT encoding card vary according to the server. You need to change the value of `NETINT` in the `hardware_bind.cfg` file based on [4](#li5561723173614) and your environment to avoid performance deterioration due to cross-chip encoding.
4. To enable hardware decoding of the Quadra encoding card, set `T432_QUADRA_DECODE_ENABLE` to `1` in the `cfct_config` file.
...
8. If the CPU is used for software encoding of video frames, set `CPU_BIND_MODE` to `1` in `cfct_config` to prevent frame freezing.
9. To enable the graphics acceleration layer, see [Basic Functions and Usage of the Graphics Acceleration Layer](#section9932195417616).
10. To enable the C2 decoder, set `ENABLE_AMD_C2_DECODE` in the `cfct_config` file to `1`.
...
4. <a name="li5561723173614"></a>Change the value of `NETINT` in `hardware_bind.cfg` based on the NUMA information corresponding to the NVMe node of the encoding card.
...
#### Setting cfct_config and hardware_bind.cfg (Configuration Schemes 2, 3, 4, and 5)<a name="ZH-CN_TOPIC_0000002518386592"></a>
...
5. If only one GPU is used in your environment, change the value of `VIDEO_CPU_MAP_{total_CPU_core_count}CORE_MODE{CPU_BIND_MODE}` in the `hardware_bind.cfg` file.
...
6. To enable the graphics acceleration layer, see [Basic Functions and Usage of the Graphics Acceleration Layer](#basic-functions-and-usage-of-the-graphics-acceleration-layer).

#### Creating a Base Data Volume<a name="ZH-CN_TOPIC_0000002518386594" id="creating-a-base-data-volume"></a>
...
### Video Stream Cloud Phone Deployment in a Kubernetes Cluster (Configuration Scheme 2)<a name="ZH-CN_TOPIC_0000002518226646"></a>

#### Preparing the Environment<a name="ZH-CN_TOPIC_0000002549866425"></a>
...
#### Setting Up a Kubernetes Cluster<a name="ZH-CN_TOPIC_0000002518386560"></a>

##### Common Operations on All Nodes<a name="ZH-CN_TOPIC_0000002518386570"></a>
...
##### Operations on the Master Node<a name="ZH-CN_TOPIC_0000002549746441"></a>
...
##### Operations on a Worker Node<a name="ZH-CN_TOPIC_0000002549746443" id="operations-on-a-worker-node"></a>
...
#### Deploying Images<a name="ZH-CN_TOPIC_0000002549746431"></a>

##### Deploying the DaoCloud Device Plugin Image<a name="ZH-CN_TOPIC_0000002518386564" id="deploying-the-daocloud-device-plugin-image"></a>
...
1. Obtain the `VAGPU-A15-C-F-26.02.06.00.RC2.tgz` GPU driver package according to "Software Environment" in [Kbox Cloud Phone Container Installation Guide (Android 15)](https://www.hikunpeng.com/document/detail/en/kunpengcps/boostcph/kboxcpc_ad15/docs/en/install_guide.md#d22-software-environment). Decompress the package to obtain the `k8s/v0.0.5-1.tar.gz` package.
2. Decompress `v0.0.5-1.tar.gz` to obtain installation documents and software packages.
3. Install Va Docker based on chapter 4 in the *DC1000 Accelerator Card Va Docker Installation Guide  02*.
...
##### Deploying the Device Plugin Image<a name="ZH-CN_TOPIC_0000002518226672"></a>
...
##### Deploying the Video Stream Image<a name="ZH-CN_TOPIC_0000002549746415"></a>
...
## VM Environment Deployment<a name="ZH-CN_TOPIC_0000002550093497"></a>

### Environment Requirements<a name="ZH-CN_TOPIC_0000002518653638"></a>
...
|Item|Version|How to Obtain|
|--|--|--|
|openEuler|24.03 LTS SP1|[Link](https://www.openeuler.openatom.cn/en/download/archive/detail/?version=openEuler%2024.03%20LTS%20SP1)|
| Kernel | Based on 6.6.0-72.0.0| For details, see "Software Deployment > Compiling the Kernel" in the *Kbox Cloud Phone Container Installation Guide (Android 15)*.|
...
### Host Environment Configuration<a name="ZH-CN_TOPIC_0000002518493742"></a>

#### Modifying BIOS Settings<a name="ZH-CN_TOPIC_0000002549973491"></a>
...
#### Modifying the Kernel Module<a name="ZH-CN_TOPIC_0000002550093499"></a>
...
4. Obtain the kernel patch file `general.patch` according to [Software Requirements](#software-requirements).
...
#### Installing VM Dependencies<a name="ZH-CN_TOPIC_0000002518493744"></a>
...
#### Querying PCIe Node Information of a GPU<a name="ZH-CN_TOPIC_0000002549973493"></a>
...
1. <a name="zh-cn_topic_0000002518185514_p644mcpsimp"></a>Check the IDs of all PCIe nodes of the DaoCloud GPUs.
...
#### Configuring the Host Network<a name="ZH-CN_TOPIC_0000002550093501" id="configuring-the-host-network"></a>
...
### VM Configuration<a name="ZH-CN_TOPIC_0000002518653642"></a>

#### Creating a VM Using virt-manager<a name="ZH-CN_TOPIC_0000002518493746"></a>
...
7. Set **Name** to **vm***X* (*X* = 0, 1, 2, or 3). If the bridge mode is configured for the host machine in [Configuring the Host Network](#configuring-the-host-network), select **Bridge device...** from **Network selection**, enter the bridge name, and click **Finish**.

    ![](figures/zh-cn_image_0000002549705529.png)

    >![](public_sys-resources/icon-note.gif) **NOTE**
    >
    >If SR-IOV is selected in [Configuring the Host Network](#configuring-the-host-network), you do not need to configure **Network selection**. These virtual network interfaces will be deleted later.
...
9. On the following page, click **Add Hardware** and add the following devices in sequence.
    ...
    3. Add GPU PCIe devices (each VM (4 VMs in total) requires two GPUs and thus eight GPU nodes need to be added in sequence): **Add Hardware > PCI Host Device > ***Select a node*** > Finish**
    4. If the SR-IOV solution is used in [Configuring the Host Network](#configuring-the-host-network), add a vNIC to the VM: **Add Hardware > PCI Host Device > ***Select a node*** > Finish**. For details about NIC nodes, see the command output in [Configuring the Host Network-5](#configuring-the-host-network-5).
...
#### Configuring the VM Network<a name="ZH-CN_TOPIC_0000002550093503"></a>

##### Configuring the VM NIC Configuration File<a name="ZH-CN_TOPIC_0000002518653644"></a>
...
### Video Stream Startup Environment Configuration (VM)<a name="ZH-CN_TOPIC_0000002550093505"></a>

After deploying cloud phones and video stream containers in the VM environment, you need to modify the `hardware_bind.cfg` file based on the number of VM CPU and GPU cores to correctly set the video stream startup and running parameters.

Deploy the cloud phone container environment on the VM and run video stream cloud phones. For details, see [Hardware Environment](https://www.hikunpeng.com/document/detail/en/kunpengcps/boostcph/kboxcpc_ad15/docs/en/install_guide.md#d21-hardware-environment) in the *Kbox Cloud Phone Container Installation Guide (Android 15)* and [Software Deployment](https://www.hikunpeng.com/document/detail/en/kunpengcps/boostcph/videostreamengine_ad15/docs/en/install_guide.md#d1-software-deployment) in the *Video Stream Engine Installation Guide (Android 15)*.

The specific steps are as follows:

1. Extract the `cfct_video` and `hardware_bind.cfg` files. For details, see [Software Deployment](https://www.hikunpeng.com/document/detail/en/kunpengcps/boostcph/videostreamengine_ad15/docs/en/install_guide.md) in the *Video Stream Engine Installation Guide (Android 15)*.
