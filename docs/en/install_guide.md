...
## 1 Software Deployment<a name="ZH-CN_TOPIC_0000002549826277"></a>

### 1.1 Environment Requirements<a name="ZH-CN_TOPIC_0000002549706275"></a>

#### 1.1.1 Hardware Environment<a name="ZH-CN_TOPIC_0000002550361171"></a>
...
#### 1.1.2 Software Environment<a name="ZH-CN_TOPIC_0000002549706295"></a>

##### 1.1.2.1 Kbox<a name="ZH-CN_TOPIC_0000002549826303"></a>

For details about the software environment for deploying the Kbox cloud phone container, see "Software Environment" in [Kbox Cloud Phone Container Installation Guide](https://www.hikunpeng.com/document/detail/en/kunpengcps/boostcph/kboxcpc/docs/en/install_guide.md#d22-software-environment-).
...
|9|WebClient.zip|Web client.|Please submit an ISSUE feedback.|√|√|√|√|
...
### 1.2 Deploying a Cloud Phone<a name="ZH-CN_TOPIC_0000002549826291"></a>

#### 1.2.1 Environment Configuration<a name="ZH-CN_TOPIC_0000002549826289"></a>

##### 1.2.1.1 Configuring the BIOS<a name="ZH-CN_TOPIC_0000002518346454"></a>
...
For details, see "Configuring the BIOS" in [Kbox Cloud Phone Container Installation Guide](https://www.hikunpeng.com/document/detail/en/kunpengcps/boostcph/kboxcpc/docs/en/install_guide.md#d41-inserting-dimms).
...
- **[(Configuration Scheme 1) Configuring the GPU Working Mode and CPU Binding](https://gitcode.com/boostkit/Kbox/blob/AOSP11/docs/en/install_guide.md#configuring-the-gpu-working-mode-and-cpu-binding)** 
...
For details about how to deploy a Kbox cloud phone container, see [Kbox Cloud Phone Container Installation Guide](https://www.hikunpeng.com/document/detail/en/kunpengcps/boostcph/kboxcpc/docs/en/install_guide.md). Perform all operations before section "Starting and Uninstalling a Cloud Phone Instance" in this chapter.
...
- **[(Configuration Scheme 2/3/4) Installing the GPU Driver](https://gitcode.com/boostkit/Kbox/blob/AOSP11/docs/en/install_guide.md#installing-the-gpu-driver)**  

#### 1.2.2 Creating an Image<a name="ZH-CN_TOPIC_0000002549826281"></a>

##### 1.2.2.1 Creating a Kbox Image<a name="ZH-CN_TOPIC_0000002549826313"></a>
...
2. Decompress the `Kbox-patches-AOSP11.zip` package, extract the `base_box.sh` file from `deploy_scripts` to the`/home/kbox_video` directory, and grant permissions on the file. Ensure that the file owner has the read, write, and execute permissions while users in the owner group and others have only the read and execute permissions.
...
3. Create a Kbox image named `kbox:origin`.
    ...
    4. (Hardware configuration scheme 2/3/4) If hardware configuration scheme 2/3/4 is used, decompress the GPU driver package `VAGPU-25.03.01.01-RC20.tgz` to obtain `va_driver.tgz` and upload `va_driver.tgz` to the `~/dependency/deploy_scripts` directory on the server.
...
##### 1.2.2.2 Creating a Video Stream Cloud Phone Image<a name="ZH-CN_TOPIC_0000002549706277"></a>
...
#### 1.2.3 Setting the cfct_config File (Configuration Scheme 1)<a name="ZH-CN_TOPIC_0000002549706303"></a>
...
#### 1.2.4 Setting the cfct_config File (Configuration Schemes 2, 3, and 4)<a name="ZH-CN_TOPIC_0000002518186514"></a>
...
3. By default, the hardware decoding function of the DC1000 GPU is enabled on the current video stream cloud phone (`ENABLE_HARD_DECODE=1`). To use software decoding, set `ENABLE_HARD_DECODE=0` and restart the container.
4. To enable the WebRTC feature, set `ENABLE_WEBRTC_CONNECTION` to `1` in `cfct_config`.
5. Bind cores and confirm that the core binding takes effect. If only one GPU is used in your environment, change the value of `VIDEO_CPU_MAP_{total_CPU_core_count}CORE_MODE{CPU_BIND_MODE}` in the `cfct_config` file.
...
#### 1.2.5 Creating a Base Data Volume<a name="ZH-CN_TOPIC_0000002518346430" id="creating-a-base-data-volume"></a>
...
### 1.3 Video Stream Cloud Phone Deployment in a Kubernetes Cluster (Configuration Scheme 2)<a name="ZH-CN_TOPIC_0000002518346436"></a>

#### 1.3.1 Preparing the Environment<a name="ZH-CN_TOPIC_0000002518346440"></a>
...
|k8s-slave1|Worker node|≥ 1|Deploy the video stream cloud phone environment based on section "Software Deployment".|Runs cloud phone services.|
...
#### 1.3.2 Setting Up a Kubernetes Cluster<a name="ZH-CN_TOPIC_0000002549826297"></a>

##### 1.3.2.1 Common Operations on All Nodes<a name="ZH-CN_TOPIC_0000002549706309"></a>
...
##### 1.3.2.2 Operations on the Master Node<a name="ZH-CN_TOPIC_0000002549706297"></a>
...
##### 1.3.2.3 Operations on a Worker Node<a name="ZH-CN_TOPIC_0000002518186532"></a>
...
#### 1.3.3 Deploying Images<a name="ZH-CN_TOPIC_0000002518346452"></a>

##### 1.3.3.1 Deploying the DaoCloud Device Plugin Image<a name="deploying-the-daocloud-device-plugin-image"></a>
...
##### 1.3.3.2 Deploying the Device Plugin Image<a name="ZH-CN_TOPIC_0000002518186506"></a>
...
##### 1.3.3.3 Deploying the Image of the Plugin for Writing the Input Device Permission<a name="ZH-CN_TOPIC_0000002549706311"></a>

Deploy the image of the plugin for writing the input device permission on all worker nodes.

After the video stream cloud phone is started, a virtual input device is generated by the vinput kernel module. To access the virtual input device in the container, you need to configure its permission. This plugin automatically writes the permission of the virtual input device after the container is created.
...
##### 1.3.3.4 Deploying the Video Stream Image<a name="ZH-CN_TOPIC_0000002518186530" id="deploying-the-video-stream-image"></a>
...
## 2 VM Environment Deployment<a name="ZH-CN_TOPIC_0000002518304958"></a>

### 2.1 Environment Requirements<a name="ZH-CN_TOPIC_0000002549944725"></a>

You are advised to set up the VM environment of the video stream engine on openEuler 22.03 LTS SP4 running on the new Kunpeng 920 processor model. Before setting up the environment, ensure that your hardware environment meets the requirements.
...
|--|--|--|
|openEuler|22.03 LTS SP4|[Link](https://www.openeuler.org/en/download/archive/detail/?version=openEuler%2022.03%20LTS%20SP4)|
...
|--|--|--|
|openEuler|22.03 LTS SP4|[Link](https://www.openeuler.org/en/download/archive/detail/?version=openEuler%2022.03%20LTS%20SP4)|
|Kernel|5.10.0-216.0.0|For details, see "Software Deployment > Compiling the Kernel" in the *Kbox Cloud Phone Container Feature Guide*.|
...
### 2.2 Host Environment Configuration<a name="ZH-CN_TOPIC_0000002550064713" id="host-environment-configuration"></a>

#### 2.2.1 Modifying BIOS Settings<a name="ZH-CN_TOPIC_0000002518464882"></a>
...
#### 2.2.2 Modifying the Kernel Module<a name="ZH-CN_TOPIC_0000002518304960"></a>
...
#### 2.2.3 Modifying GRUB Settings<a name="ZH-CN_TOPIC_0000002549944727"></a>
...
#### 2.2.4 Installing VM Dependencies<a name="ZH-CN_TOPIC_0000002550064715"></a>
...
#### 2.2.5 Querying PCIe Node Information of a GPU<a name="ZH-CN_TOPIC_0000002518464884" id="querying-pcie-node-information-of-a-gpu"></a>
...
1. <a name="pcie-info-query-1"></a>Check the IDs of all PCIe nodes of the Hantro GPUs.
...
#### 2.2.6 Configuring the Host Network<a name="ZH-CN_TOPIC_0000002518304962" id="configuring-the-host-network"></a>
...
### 2.3 VM Configuration<a name="ZH-CN_TOPIC_0000002549944729" id="vm-configuration"></a>

#### 2.3.1 Creating a VM Using virt-manager<a name="ZH-CN_TOPIC_0000002550064717"></a>
...
3. <a id="tuning-vm-configurations-3"></a>Add the following content under &lt;/cputune&gt; to bind the VM to the NUMA memory of the host. (In this example, NUMA 1 is bound to the VM.)
...
#### 2.3.3 Configuring the VM Network<a name="ZH-CN_TOPIC_0000002518304964"></a>

##### 2.3.3.1 Configuring the VM NIC Configuration File<a name="ZH-CN_TOPIC_0000002549944731"></a>
...
### 2.4 Video Stream Startup Environment Configuration (VM)<a name="ZH-CN_TOPIC_0000002549947651" id="video-stream-startup-environment-configuration-vm"></a>
...
Deploy the cloud phone container environment on the VM and run video stream cloud phones. For details, see [Installation Guide](https://www.hikunpeng.com/document/detail/en/kunpengcps/boostcph/kboxcpc/docs/en/install_guide.md) in the *Kbox Cloud Phone Container Feature Guide* and [Software Deployment](https://www.hikunpeng.com/document/detail/en/kunpengcps/boostcph/videostreamengine/docs/en/install_guide.md#d11-environment-requirements) in the *Video Stream Engine Feature Guide*.

The specific steps are as follows:

1. Extract the `cfct_video` and `cfct_config` files. For details, see [Software Deployment](https://www.hikunpeng.com/document/detail/en/kunpengcps/boostcph/videostreamengine/docs/en/install_guide.md#d11-environment-requirements) in the *Video Stream Engine Feature Guide*.
...
### 2.5 VM Copying<a name="ZH-CN_TOPIC_0000002518304966" id="vm-copying"></a>

#### 2.5.1 Copy Description<a name="ZH-CN_TOPIC_0000002549944733"></a>
...
- If one VM has been created based on [2.3-VM Configuration](#vm-configuration), copy the VM based on the following sections to create another three VMs.
...
#### 2.5.2 Copying the Virtual Drive and Configuration File<a name="ZH-CN_TOPIC_0000002550064721"></a>
...
6. Check whether the VM is created.
...
#### 2.5.3 Tuning VM Configurations<a name="ZH-CN_TOPIC_0000002518464890"></a>
...
#### 2.5.4 Configuring the Network<a name="ZH-CN_TOPIC_0000002518304968"></a>
...
#### 2.5.5 Expanding the Virtual Drive<a name="ZH-CN_TOPIC_0000002549944735"></a>
