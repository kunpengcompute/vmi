# Compilation Script Introduction

<!-- md-trans-meta sourceCommit=4da158845ec146a473ea42c5c35b986c11a245e8 translatedAt=2026-09-08T02:25:39.939Z pushedAt=2026-09-08T06:07:33.474Z -->

This document describes the functions and implementation details of the scripts used during compilation, helping developers understand how the compilation toolchain works.
...
1. Download the compilation tool packages to ${installation_directory}. If not specified, the installation directory defaults to `~/NativeCompileToolsDir`.
2. Extract the compilation tools within ${installation_directory}.
3. Configure the environment variables in the `~/.bashrc` file to map the path to each compilation tool to its corresponding directory under ${installation_directory}.
...
If `#tools_env_end:` is found before `#tools_env_start:`, the script cannot automatically delete the content, and you must resolve the conflict manually.
...
Appends the environment variables to `~/.bashrc`, starting with the `#tools_env_start:` marker and ending with the `#tools_env_end:` marker.
...
### Introduction to opensource_repo.conf<a id="opensource_repo.conf"></a>

The `opensource_repo.conf` file records the download URLs and extraction methods for open-source software. It is called every time `download_open_source.sh` is executed.
...
Extracts the packages into the `unpack_open_source` directory. The extraction path is determined by parameter [2] and parameter [3] configured in `opensource_repo.conf`. For details, see the [Introduction to opensource_repo.conf](#opensource_repo.conf) section.
