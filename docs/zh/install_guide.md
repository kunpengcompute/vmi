...
## 部署说明<a name="ZH-CN_TOPIC_0000002518464880"></a>
...
##### 视频流引擎<a name="ZH-CN_TOPIC_0000002518346424"></a>
...
环境部署指定的服务器BIOS版本对内存的插入格式有限制。在进行BIOS设置之前，请确保内存插入格式正确，然后根据硬件配置方案需求完成MISC、Performance和Memory等相关选项的配置，用以提高服务器性能。
- **[（硬件配置方案一，可选）升级NVMe固件版本](data01-PA_TEMP-38-zh-cn_bookmap_0000002521528920260212101705869-temp-zh-cn_topic_0000002518186510.md)**  

- **[（硬件配置方案一）配置GPU工作模式和CPU绑定](data01-PA_TEMP-38-zh-cn_bookmap_0000002521528920260212101705869-temp-zh-cn_topic_0000002518346432.md)**  

##### 部署Kbox容器基础环境<a name="ZH-CN_TOPIC_0000002518186516"></a>
...
##### （可选）部署Containerd环境<a name="ZH-CN_TOPIC_0000002549826283"></a>
...
1. <a name="li1833813468392"></a>下载并解压Containerd二进制软件包到“/usr/local“目录下。
...
5. <a name="li15339114613395"></a>下载并安装nerdctl工具。
...
6. 重新启动Docker服务，并重新启动一个新的终端以使新的容器运行时生效。
    ...
    若要切换至Docker版本的容器运行时，须将[1](#li1833813468392)\~[5](#li15339114613395)已安装的相关软件二进制从对应目录中移除。移除完成后，参考以上命令重启Docker服务并重新启动一个新的终端。

视频流云手机除支持以基于Docker的容器运行时启动外，还支持以Containerd容器运行时启动。若选择以Containerd运行时运行视频流云手机，可参考本章节进行Containerd相关软件包的部署。
- **[（硬件配置方案二、三）安装显卡驱动](data01-PA_TEMP-38-zh-cn_bookmap_0000002521528920260212101705869-temp-zh-cn_topic_0000002518186526.md)**  
...
1. 请参见[部署Kbox容器基础环境](部署Kbox容器基础环境.md)获取Kbox容器启动依赖组件android.tar和Kbox-AOSP11.zip，并上传至服务器的“/home/kbox\_video“目录（本文以此目录作为示例，用户也可自行设置目录）。
2. 解压Kbox-AOSP11.zip，获取“deploy\_scripts“路径下的组件base\_box.sh，并将其拷贝到“/home/kbox\_video“目录，赋予文件权限，使文件拥有者有读、写、执行权限而属组用户和其他用户只有读和执行权限。
...
1. 请参见[视频流引擎](视频流引擎.md)获取CloudPhoneApk.tar.gz、DemoVideoEngine.tar.gz和BoostKit-boostcph-videoengine\_\*.zip软件包，获取后将软件包上传至服务器的“/home/kbox\_video“目录。
...
4. （硬件配置方案一）使用硬件配置方案一时请参见[视频流引擎](视频流引擎.md)获取NETINT-vXXX.tar.gz软件包，获取后将软件包上传至服务器的“/home/kbox\_video“目录，并重命名为NETINT.tar.gz。
...
2. 制作视频流云手机镜像。镜像名称可以使用默认镜像名也可以自己指定。
    ...
    - 如果镜像名称需要自定义，执行命令如下例所示。通过参数指定Kbox基础云手机和视频流云手机镜像名称，格式均为“\{镜像名\}:\{tag\}”，如下例所示kbox、video为镜像名，origin、latest为tag。
        ...
        >![](public_sys-resources/icon-note.gif) **说明：** 
        >镜像名只可包含数字与小写字母，首字符应为小写字母，tag名只可包含数字与字母。若自定义修改了视频流云手机的镜像名称，请参见[制作基础数据卷](制作基础数据卷.md)章节中将cfct\_config配置文件中的视频流云手机镜像名更新为自定义的镜像名称。
...
4. 创建“containerd\_config”配置文件使脚本可识别启动运行时为Containerd。
    ...
    若要切换Docker作为视频流云手机的默认容器运行时，请将“containerd\_config”文件删除。
...
#### 设置cfct\_config配置文件（配置方案一）<a name="ZH-CN_TOPIC_0000002549706303"></a>
...
**cfct\_config文件配置步骤<a name="section9436102613100"></a>**
...
3. NETINT编码卡的节点在不同服务器中会有区别，应根据实际情况修改cfct\_config中NETINT的值，保证编码不会因跨片导致性能损失。
4. 如果要使能Quadra/T432编码卡硬解，需要将cfct\_config中的“T432\_QUADRA\_DECODE\_ENABLE“设置为“1“。
5. 针对1张GPU卡环境：需要修改cfct\_config配置文件中VIDEO\_CPU\_MAP\_\{_CPU总核数_\}CORE\_MODE\{_CPU\_BIND\_MODE变量值_\}。NETINT编码卡芯片节点所属NUMA查询方式请参见[NETINT编码卡芯片节点所属NUMA查询方式](#section2507154233510)。
...
6. 针对1张编码卡环境：需要修改cfct\_config配置文件中“VIDEO\_ENC\_MAP\_CORE“。
...
9. 如果需要使能图形加速层功能，需要将cfct\_config中的“**ENABLE\_RENDER\_LAYER**”设置为“1“。详细说明请参见[图形加速层的基本功能和使用说明](#section1156712526252)。
...
4. 根据编码卡NVMe设备节点对应的NUMA修改cfct\_config中NETINT的值。
...
**图形加速层的基本功能和使用说明<a name="section1156712526252"></a>**
...
1. 将云手机启动配置文件cfct\_config中的“**ENABLE\_RENDER\_LAYER**”设置为“1“。
2. 从软件包Kbox-AOSP11.zip中复制kbox\_render\_accelerating\_configuration.xml配置文件到启动路径“/home/kbox\_video/“。

    ```
    cp /home/kbox_video/Kbox-AOSP11/deploy_scripts/kbox_render_accelerating_configuration.xml /home/kbox_video/
    ```

3. 打开kbox\_render\_accelerating\_configuration.xml配置文件，对应用的图形加速层功能进行配置。具体配置项描述请参见[zh-cn\_topic\_0000002549706301.md](zh-cn_topic_0000002549706301.md)章节。
...
通过设置cfct\_config配置文件可以灵活配置视频流云手机使用的资源，使性能达到最优。云手机启动时必须在启动路径下存放cfct\_config配置文件，云手机容器会使用该文件中的配置，使用时应确保cfct\_config配置文件中的配置正确。
#### 设置cfct\_config配置文件（配置方案二、三）<a name="ZH-CN_TOPIC_0000002518186514"></a>
...
3. 当前视频流云手机默认使能DC1000 GPU硬解的硬解功能（即默认**ENABLE\_HARD\_DECODE=1**），如需使用软解，需设置**ENABLE\_HARD\_DECODE=0**并重启容器。
4. 如果要使能WebRTC特性，需要更改cfct\_config中的ENABLE\_WEBRTC\_CONNECTION=1。
5. 绑核和确认绑核生效。针对1张GPU卡环境：需要修改cfct\_config配置文件中VIDEO\_CPU\_MAP\_\{_CPU总核数_\}CORE\_MODE\{_CPU\_BIND\_MODE变量值_\}。
...
6. 如果要使能图形加速层，需要将cfct\_config中“ENABLE\_RENDER\_LAYER“设置为“1“。详细说明请参见[图形加速层的基本功能和使用说明](设置cfct_config配置文件（配置方案一）.md#section1156712526252)。

通过设置cfct\_config配置文件可以灵活配置视频流云手机使用的资源，使性能达到最优。云手机启动时必须在启动路径下存放cfct\_config配置文件，云手机容器会使用该文件中的配置，使用时应确保cfct\_config配置文件中的配置正确。
#### 制作基础数据卷<a name="ZH-CN_TOPIC_0000002518346430"></a>
...
4. 使用cfct\_video脚本启动1路云手机，本文以android\_1为例。
...
7. 请参见[（可选）部署Containerd环境](（可选）部署Containerd环境.md)的[1](（可选）部署Containerd环境.md#li1833813468392)至[3](（可选）部署Containerd环境.md#li163391469397)安装Containerd和runc组件。在完成Containerd和runc的组件安装后，工作节点需要额外执行[6](（可选）部署Containerd环境.md#li84947432146)进行Docker服务的重启。
8. 修改Containerd配置。
...
2. 启动kube-flannel网络插件。
    ...
    请参见[视频流引擎](视频流引擎.md)获取DemoVideoEngine.tar.gz软件包，获取后将软件包上传至服务器的“/home/k8s”目录。
...
5. （可选）配置NUMA亲和。
    ...
    2. 请参见[视频流引擎](视频流引擎.md)获取K8s NUMA亲和插件软件包topo-affinity-plugin-master.zip，获取后将软件包上传至服务器的“/home/k8s“目录。
...
##### 部署道客设备插件镜像<a name="ZH-CN_TOPIC_0000002518346450"></a>

在所有工作节点完成部署道客设备插件镜像的操作。

道客设备插件由道客提供，本文档配套v0.0.5版本。请先获取相关的安装文档和软件包，并按照文档完成道客设备插件的部署。

1. 请参见《[Kbox云手机容器 特性指南](https://www.hikunpeng.com/document/detail/zh/kunpengcps/cpturbokit/kboxcpc/kunpengcpskbox_20_0131.html)》中软件部署的“环境准备”章节获取显卡驱动VAGPU-25.03.01.01-RC20.tgz软件包。解压获取k8s-v0.0.5-1.tar.gz压缩包。
...
在所有工作节点完成部署input设备权限写入插件镜像的操作。
##### 部署视频流镜像<a name="ZH-CN_TOPIC_0000002518186530"></a>
...
2. <a name="li138583264566"></a>修改编码器类型，重新制作DemoVideoEngine.tar.gz软件包。
...
3. 使用[2](#li138583264566)制作的DemoVideoEngine.tar.gz，请参见[制作镜像](制作镜像.md)重新制作视频流镜像。例如：制作出的镜像名为video:version。
4. 使用**docker**导出视频流镜像。
...
**获取软件包<a name="section1543425619147"></a>**
...
### 宿主机环境配置<a name="ZH-CN_TOPIC_0000002550064713"></a>
...
3. 请参见[获取软件包](zh-cn_topic_0000002549705255.md#section1543425619147)，获取内核patch文件general.patch。
...
2. 按“i”进入编辑模式，在GRUB\_CMDLINE\_LINUX后添加如下所示配置。
...
8. 查看内存大页是否成功配置。
    ...
    回显如下图所示，“Node  _x_  HugePages\_Total”字段总和为“800“则为配置成功。
...
#### 查询GPU卡PCIe节点信息<a name="ZH-CN_TOPIC_0000002518464884"></a>
...
1. <a name="p644mcpsimp"></a>确认瀚博GPU卡所有PCIe节点的ID。
...
2. 确认所对应的NUMA，此返回顺序符合[1](#p644mcpsimp)中的ID顺序，即可确定每个GPU卡节点所对应的NUMA ID。下图所示回显信息仅为示例，如17:00.0\~18:00.3（即前八个节点）对应宿主机的NUMA 1。
...
#### 配置宿主机网络<a name="ZH-CN_TOPIC_0000002518304962"></a>

创建宿主机网络设备，支撑后续虚拟机网络配置。

1. <a name="li82551838113215"></a>查看宿主机使用的网卡。
...
3. <a name="li1122704216353"></a>查看该网卡最多支持的VF网卡数量。
    ...
    - 如果该步骤执行失败，或者回显的虚拟网卡数小于准备部署的虚拟机数量，则可以选择第二个方案网桥模式。网桥模式会带来额外的计算性能损耗以及时延。网桥模式配置详情请查看[6](#li9644845123820)\~[7](#li55473504593)。
...
5. <a name="li115459266401"></a>查看生成的VF节点。
    ...
    >![](public_sys-resources/icon-note.gif) **说明：** 
    >[1](#li82551838113215)\~[5](#li115459266401)已经完成了SR-IOV虚拟网卡方案中，虚拟网卡的生成。后续步骤可以跳过。
    >如果设备不支持SR-IOV，考虑使用下面的网桥方案。
    >如果[3](#li1122704216353)中网卡最多支持的VF网卡数量回显小于4。例如为2，那么考虑2个虚拟机使用SR-IOV方案，2个虚拟机使用网桥方案。

6. <a name="li9644845123820"></a>查看当前网卡配置文件并备份。
...
7. <a name="li55473504593"></a>新建网桥配置文件“ifcfg-br0“并修改网卡配置文件。
...
4. <a name="li664mcpsimp"></a>“Memory“分配额度填写“180000“，“CPUs“处填写“80“，然后单击“Forward“。

    ![](figures/1_zh-cn_image_0000002085084878.png)

    >![](public_sys-resources/icon-note.gif) **说明：** 
    >内存180000仅供参考。建议根据[8](zh-cn_topic_0000002549825257.md#li622mcpsimp)中内存大页分配情况，将NUMA分配的内存大页全部分配到对应的虚拟机中。
...
6. 名称分别起名为vm_X_（X = 0,1,2,3） ，同时需要勾选“Customize configuration before install”，如果[配置宿主机网络](zh-cn_topic_0000002549705297.md)中宿主机配置了网桥模式，此处Network selection应选择Bridge br0，然后单击“Finish“。

    ![](figures/zh-cn_image_0000002518308370.png)

    >![](public_sys-resources/icon-note.gif) **说明：** 
    >如果[配置宿主机网络](zh-cn_topic_0000002549705297.md)选择了SR-IOV虚拟网卡直通配置，则无需考虑Network selection，后续会删除这些虚拟网络接口。
...
7. 进入界面后单击下方“Add Hardware“依次添加以下相关设备。
    ...
    3. 添加GPU卡PCIe设备（每个虚拟机需要2张GPU卡，因此依次添加8个节点） :  “Add Hardware \> PCI Host Device  \> 选择对应节点 \> Finish“。GPU节点和[2](zh-cn_topic_0000002518345416.md#li1975019467143)中的NUMA节点需与[查询GPU卡PCIe节点信息](zh-cn_topic_0000002549825267.md)对应选择。
    4. 如果[配置宿主机网络](zh-cn_topic_0000002549705297.md)中采用了SR-IOV方案，添加虚拟网卡到虚拟机：“Add Hardware \> PCI Host Device  \> 选择对应节点 \> Finish“。网卡节点请参见[5](zh-cn_topic_0000002549705297.md#li115459266401)回显信息。
...
13. 在开始安装前查看服务器当前空闲的内存。
    ...
    如果空闲内存小于[4](#li664mcpsimp)中设置的虚拟机内存，则提前执行[3](zh-cn_topic_0000002518345416.md#li16990103861511)和[5](zh-cn_topic_0000002518345416.md#li39871559111511)，请参见[获取软件包](zh-cn_topic_0000002549705255.md#section1543425619147)获取虚拟机调优脚本setup\_vm.sh，使能虚拟机内存大页。如果空闲内存足够，可以跳过该步骤。
...
#### 虚拟机配置调优<a name="ZH-CN_TOPIC_0000002518464886"></a>

虚拟机配置调优需要根据GPU和NUMA的对应关系进行。

**此小节4个虚拟机均需执行，本章节根据GPU与NUMA的对应关系示例vm0对应NUMA 1进行修改。**请参见[查询GPU卡PCIe节点信息](zh-cn_topic_0000002549825267.md)查询GPU与NUMA的对应关系，请参见[获取软件包](zh-cn_topic_0000002549705255.md#section1543425619147)获取虚拟机调优脚本。
...
2. 生成配置文件。
    ...
    - 如果在[配置宿主机网络](zh-cn_topic_0000002549705297.md)章节中选择了SR-IOV网卡直通，则执行如下指令，生成网络配置文件。
    ...
    - 如果[配置宿主机网络](zh-cn_topic_0000002549705297.md)中采用的网桥模式，那么虚拟机中会自动生成网络配置文件，文件名和**ip a**中网卡名称一致，则跳过该步骤。
...
##### （可选）修改SSH端口<a name="ZH-CN_TOPIC_0000002550064719"></a>
...
##### （可选）配置路由表<a name="ZH-CN_TOPIC_0000002518464888"></a>
...
### 视频流启动环境配置（虚拟机）<a name="ZH-CN_TOPIC_0000002549947651"></a>

在搭建好的虚拟机环境中部署云手机容器环境和视频流容器时，需要根据虚拟机内部CPU以及GPU核数对cfct\_video和cfct\_config文件做相应调整和修改视频流启动和配置文件。
...
1. 请参见《视频流引擎 特性指南》的“[软件部署](https://www.hikunpeng.com/document/detail/zh/kunpengcps/cpturbokit/videostreamengine/kunpengcpsvideo_20_0048.html)”解压缩出cfct\_video和cfct\_config文件。
...
4. 请参见《视频流引擎 特性指南》的“启动视频流云手机”调用cfct\_video脚本即可成功在虚拟机启动视频流容器。

    ![](figures/zh-cn_image_0000002518308398.png)


### 虚拟机拷贝<a name="ZH-CN_TOPIC_0000002518304966"></a>

#### 拷贝说明<a name="ZH-CN_TOPIC_0000002549944733"></a>

- 如果已经按照[虚拟机配置](zh-cn_topic_0000002518185512.md)顺序创建了4个虚拟机，则跳过本章节之后的所有操作。
- 如果按照[虚拟机配置](zh-cn_topic_0000002518185512.md)创建了1个虚拟机，则使用本章节后续内容拷贝创建虚拟机剩下的3个虚拟机。

    推荐在完成[宿主机环境配置](zh-cn_topic_0000002549825277.md)、[虚拟机配置](zh-cn_topic_0000002518185512.md)、[视频流启动环境配置（虚拟机）](zh-cn_topic_0000002549825289.md)后进行虚拟机的拷贝，可以避免创建新虚拟机时进行大量重复配置。若完成了上述章节，拷贝后的虚拟机仅需对[虚拟机配置](zh-cn_topic_0000002518185512.md)中少量配置进行调整后即可正常使用。
...
7. 请参见[查询GPU卡PCIe节点信息](zh-cn_topic_0000002549825267.md)获取虚拟机绑定NUMA对应的GPU节点。
8. 进入虚拟机删除当前GPU节点。
...
9. 请参见[查询GPU卡PCIe节点信息](zh-cn_topic_0000002549825267.md)获取当前虚拟机匹配的GPU卡的PCIe节点，再通过“Add Hardware \> PCI Host Device  \> 选择对应节点 \> Finish“，配置所有的PCIe节点。
...
10. 如果虚拟机使能SR-IOV网卡直通，需要先删除原虚拟机使用的SR-IOV虚拟网卡。
    ...
    请参见[配置宿主机网络](zh-cn_topic_0000002549705297.md)，添加新的SR-IOV虚拟网卡。
...
11. 如果虚拟机使能了磁盘分区直通，需要先删除原虚拟机直通的磁盘分区。
    ...
    请参见[15](zh-cn_topic_0000002518185480.md#li570111715325)，添加新的磁盘分区。
...
请参见[虚拟机配置调优](zh-cn_topic_0000002518345416.md)进行虚拟机配置调优。
