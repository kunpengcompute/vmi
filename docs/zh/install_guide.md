...
|--|--|--|--|--|--|--|--|--|
| 1 | BoostKit-boostcph-videoengine_*_15.zip | 视频流引擎二进制包 | [获取链接](https://gitcode.com/boostkit/boostcph/releases) | √ | √ | √ | √ |√ |
...
1. 请参见[**表 1** 部署视频流引擎的软件环境要求](#部署视频流引擎的软件环境要求)获取软件包和对应的SHA256文件。
...
5. 针对1张GPU卡环境：需要修改hardware_bind.cfg配置文件中VIDEO_CPU_MAP_{_CPU总核数_}CORE_MODE{_CPU_BIND_MODE变量值_}，`CPU_BIND_MODE`的值可以在cfct_config中找到。
