# 例行维护<a name="ZH-CN_TOPIC_0000002521735640"></a>

## 1 运维概述<a name="ZH-CN_TOPIC_0000002549706251"></a>

云手机是指虚拟出带有AOSP （Android Open Source Project）系统，具有虚拟手机功能的云服务器。作为一种新型应用，它对物理手机起到了有效的延伸和扩展作用，可以用在云手游、移动办公等诸多场景。

视频流云手机是一种基于Kbox容器和端云协同引擎的虚拟化方案。Kbox容器是指运行于ARM服务器上的Android系统Docker容器。端云协同引擎顾名思义可以分为端侧和云侧两个部分：云侧运行于云手机上；端侧一般为手机APK，可以被安装在用户的Android手机上，用于跟云侧进行交互，进而对视频流云手机进行正常的操作。视频流云手机架构图如[**图 1** 视频流云手机架构图](#视频流云手机架构图)所示。

**图 1** 视频流云手机架构图<a name="fig13904113085011"></a><a id="视频流云手机架构图"></a>
...
## 2 巡检<a name="ZH-CN_TOPIC_0000002549826261"></a>

### 2.1 简介<a name="ZH-CN_TOPIC_0000002549826265"></a>

巡检是人工进行的例行检查。通过巡检操作，可以实时掌握视频流云手机的运行状况及周围环境的变化，发现其中的隐患，及时采取有效措施，提高对突发事件的处理效率，保证其安全和系统稳定。本文档主要以Docker容器运行时为例进行相关巡检项目的演示。

### 2.2 巡检项目和周期<a name="ZH-CN_TOPIC_0000002518186486"></a>

根据例行维护巡检操作，输出如[**表 1** 视频流云手机例行维护巡检报告](#视频流云手机例行维护巡检报告)所示巡检报告。

首次视频流云手机部署完成后，建议立即进行一次巡检。

**表 1** 视频流云手机例行维护巡检报告<a id="视频流云手机例行维护巡检报告"></a>
...
### 2.3 检查运行环境状态<a name="ZH-CN_TOPIC_0000002549706243"></a>

#### 2.3.1 Docker服务状态<a name="ZH-CN_TOPIC_0000002549826263"></a>
...
### 2.4 检查视频流云手机状态<a name="ZH-CN_TOPIC_0000002518186490"></a>

#### 2.4.1 容器状态<a name="ZH-CN_TOPIC_0000002518346408"></a>

##### 2.4.1.1 启动时容器状态<a name="ZH-CN_TOPIC_0000002518346410"></a>
...
3. 执行以下命令，进入视频流云手机容器中，其中“\{android\_id\}“为启动实例的编号。

    ```shell
    docker exec -it android_${android_id} sh
    ```

4. 执行以下命令，查看sys.boot\_completed的属性状态是否得到开机状态。

    ```shell
    getprop sys.boot_completed
    ```

    若回显信息中“sys.boot\_completed“显示为“1“，则表示启动成功。

##### 2.4.1.2 运行时容器状态<a name="ZH-CN_TOPIC_0000002549826257"></a>
...
3. 执行以下命令，进入视频流云手机容器中，其中“\{android\_id\}“为自定义的启动实例的编号，一般为从1开始的数字。
...
4. 执行以下命令查看进程状态，分为两种情况：
    - 若出现大量进程的父进程变为进程sh（进程号为1），如下图所示，则说明容器已经crash，容器已经处于异常状态，请联系华为技术支持。
    ...
    - 查看VmiAgent进程是否存在，若不存在，证明VmiAgent已经被结束，视频流云手机则不能正常使用，请联系华为技术支持。

#### 2.4.2 容器资源耗能<a name="ZH-CN_TOPIC_0000002518346402"></a>
...
#### 2.4.3 引擎服务状态<a name="ZH-CN_TOPIC_0000002518346400"></a>

在视频流云手机容器运行过程中，可以通过检查其进程状态判断容器是否正常。

1. 使用视频流云手机运维用户（如root）账号通过SSH登录工具（如Xshell）登录到部署视频流云手机的服务器。
2. 执行以下命令，进入视频流云手机容器中，其中“\{android\_id\}“为启动实例的编号。
...
3. 执行以下命令查看进程状态，分为以下两种情况：
    - 查看VmiAgent进程。若VmiAgent进程不存在，证明VmiAgent已经被结束，视频流云手机则不能正常使用，请联系华为技术支持。
    ...
    - 查看VmiAgent进程。若VmiAgent进程存在，如上图所示红框，查看其对应的PID（对应上图中的283），使用以下命令可以查看进程状态、CPU使用率和内存使用率等，其中“\{pid\}“为VmiAgent进程编号，如下图所示。
...
## 3 监控<a name="ZH-CN_TOPIC_0000002549826259"></a>

### 3.1 简介<a name="ZH-CN_TOPIC_0000002549706261"></a>

监控是针对视频流云手机资源和应用的监控，通过应用监控您可以及时了解资源使用情况、趋势和告警，使用这些信息，您可以快速响应，保证云手机流畅运行。

### 3.2 主机CPU和内存使用率状态<a name="ZH-CN_TOPIC_0000002518186482"></a>
...
3. 执行以下命令，分别查询主机容器中进程的CPU及内存使用率等情况。如下图所示。
    ...
    >- USER：进程创建者。
    >- PR：进程优先级。
    >- NI：nice值。越小优先级越高，最小-20，最大20（用户最大可设置19）。
...
### 3.3 容器CPU和内存占用率状态<a name="ZH-CN_TOPIC_0000002518346390"></a>
...
3. 执行以下命令，进入Docker容器中查看相关CPU和内存占用率状态，其中“\{android\_id\}“为启动实例的编号。
...
### 3.4 系统内存<a name="ZH-CN_TOPIC_0000002518186476"></a>
...
### 3.5 监控磁盘使用率状态<a name="ZH-CN_TOPIC_0000002518186472"></a>
...
3. 执行以下命令，查看视频流云手机服务器上的磁盘空间使用率。
    ...
    上图中视频流云手机容器数据存储在“/home/mount/data/“下，红色矩形圈出来的是容器android\_1的数据存储信息，其存储大小为“16G“，当前使用率为“4%“。若“Use%“的值均不大于85%，则磁盘空间使用率正常。否则，需要处理磁盘空间。

### 3.6 监控负载<a name="ZH-CN_TOPIC_0000002518186478"></a>
...
## 4 日志管理<a name="ZH-CN_TOPIC_0000002518186484"></a>

### 4.1 简介<a name="ZH-CN_TOPIC_0000002518186492"></a>

日志管理提供强大的日志管理能力，包括日志检索、日志转储等功能。通过日志管理能够记录用户操作的审计日志，快速定位问题的根源、跟踪程序执行的过程，还能用于数据统计和性能分析等。

### 4.2 基础云手机维护工具<a name="ZH-CN_TOPIC_0000002518346404"></a>

Kbox云手机容器提供了Kbox\_maintainer（维护工具）。该工具包含日志收集、资源检查、故障恢复等功能，提升云手机原型的可测试性、可服务性、可维护性。该工具的获取和使用方法请参见《[Kbox云手机容器 例行维护（Android 15）](https://www.hikunpeng.com/document/detail/zh/kunpengcps/cpturbokit/kboxcpc_ad15/kunpengcpskbox_32_0028.html)》的“维护工具”章节。

### 4.3 视频流云手机查询日志操作<a name="ZH-CN_TOPIC_0000002549826255"></a>

#### 4.3.1 Android侧查询日志操作<a name="ZH-CN_TOPIC_0000002549826251"></a>

##### 4.3.1.1 ANR时应用堆栈信息<a name="ZH-CN_TOPIC_0000002518346392"></a>

在视频流云手机容器内若出现了应用程序未响应（ANR，Application Not responding）时，需要搜集相关应用堆栈信息，该信息保存在容器的“/data/anr/“路径下。

#### 4.3.2 服务器侧查询日志操作<a name="ZH-CN_TOPIC_0000002549706245"></a>

##### 4.3.2.1 系统日志信息<a name="ZH-CN_TOPIC_0000002549826249"></a>
...
##### 4.3.2.2 运行环境日志信息<a name="ZH-CN_TOPIC_0000002549706241"></a>
...
#### 4.3.3 Docker容器查询日志操作<a name="ZH-CN_TOPIC_0000002549826247"></a>

##### 4.3.3.1 容器元数据信息<a name="ZH-CN_TOPIC_0000002518346394"></a>
...
4. 可使用Docker提供的inspect命令查看容器的详细信息，如下命令，其中“\{android\_id\}“为启动实例的编号。
...
##### 4.3.3.2 容器内logcat日志信息<a name="ZH-CN_TOPIC_0000002549706257"></a>
...
4. 执行以下命令可以查询到实时的服务端日志信息。其中“\{android\_id\}“为对应的容器编号，“VMI“为视频流云手机的日志tag前缀。

    ```shell
    docker exec -it android_${android_id} logcat |grep -i VMI
    ```

##### 4.3.3.3 容器内进程信息<a name="ZH-CN_TOPIC_0000002518346398"></a>
...
4. 执行以下命令可以查询到当前正在运行的进程信息，其中“\{android\_id\}“为对应的容器编号。

    ```shell
    docker exec -it android_${android_id} ps -elf
    ```

##### 4.3.3.4 容器内top信息<a name="ZH-CN_TOPIC_0000002549706247"></a>
...
4. 执行以下命令，可以实时查询到容器中各个进程的资源占用情况，其中“\{android\_id\}“为对应的容器编号。

    ```shell
    docker exec -it android_${android_id} top
    ```

##### 4.3.3.5 容器属性信息<a name="ZH-CN_TOPIC_0000002549706249"></a>
...
4. 执行以下命令，可以查询到容器中系统属性信息，其中“\{android\_id\}“为对应的容器编号。

    ```shell
    docker exec -it android_${android_id} getprop
    ```

##### 4.3.3.6 容器dumpsys信息<a name="ZH-CN_TOPIC_0000002518186494"></a>
...
4. 执行以下命令，获取容器的所有系统服务诊断输出，其中“$\{android\_id\}“为启动实例的编号。
...
### 4.4 视频流云手机导出日志操作<a name="ZH-CN_TOPIC_0000002549826245"></a>
...
4. 执行以下命令，可以查询到实时的服务端日志信息，其中“\{android\_id\}“为对应的容器编号。
...
### 4.4 日志溢出转储<a name="ZH-CN_TOPIC_0000002549706253"></a>

日志溢出转储功能的具体实现取决于ISV，视频流引擎不涉及日志溢出转储的设计。

### 4.5 日志审计<a name="ZH-CN_TOPIC_0000002549706259"></a>

请参见《[视频流引擎 开发指南（Android 15）](development_guide.md)》文档中用户对视频流对外接口、对外依赖接口等设置的参数进行记录，做到定期记录，可追溯。

## 5 高危操作<a name="ZH-CN_TOPIC_0000002518186480"></a>

### 5.1 禁用操作一览表<a name="ZH-CN_TOPIC_0000002518346396"></a>

禁用操作暂无，如有疑问，请联系华为技术支持。

### 5.2 高危操作一览表<a name="ZH-CN_TOPIC_0000002518346388"></a>
...
## 6 日常运维<a name="ZH-CN_TOPIC_0000002518186474"></a>

### 6.1 简介<a name="ZH-CN_TOPIC_0000002549706255"></a>

根据项目日常操作，需要定期进行日常维护，输出如[**表 1** 视频流云手机日常运维报告](#视频流云手机日常运维报告) 视频流云手机日常运维报告](#table978519229119)所示运维报告。

首次视频流云手机部署完成后，建议立即进行一次巡检。

**表 1** 视频流云手机日常运维报告<a id="视频流云手机日常运维报告"></a>
...
### 6.2 服务器状态<a name="ZH-CN_TOPIC_0000002518346406"></a>
...
### 6.3 容器状态<a name="ZH-CN_TOPIC_0000002549826253"></a>
...
## 7 参考信息<a name="ZH-CN_TOPIC_0000002549826243"></a>
