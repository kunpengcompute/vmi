...
- 运维功能
    ...
    提供运维对象的巡检、看管/记录、日志管理、高危操作和日常维护等能力。
...
1. 使用视频流云手机运维用户（如root）账号通过SSH登录工具（如Xshell）登录到部署视频流云手机的服务器上。
...
4. <a name="li65832418453"></a>按照以下步骤重启Docker服务。重启后请重新进行服务状态查询是否为"active"状态。若重启失败或服务状态不为"active"状态，请提交ISSUE反馈。
...
4. 执行以下命令查看进程状态，分为两种情况：
    - 若出现大量进程的父进程变为进程sh（进程号为1），如下图所示，则说明容器已经crash，容器已经处于异常状态，请提交ISSUE反馈。
    ...
    - 查看VmiAgent进程是否存在，若不存在，证明VmiAgent已经被结束，视频流云手机则不能正常使用，请提交ISSUE反馈。
...
3. 执行以下命令查看进程状态，分为以下两种情况：
    - 查看VmiAgent进程。若VmiAgent进程不存在，证明VmiAgent已经被结束，视频流云手机将不能正常使用，请提交ISSUE反馈。
...
## 看管/记录<a name="ZH-CN_TOPIC_0000002549825319"></a>

### 简介<a name="ZH-CN_TOPIC_0000002549825345"></a>

看管/记录是针对视频流云手机资源和应用的看管/记录，通过应用看管/记录您可以及时了解资源使用情况、趋势和告警，使用这些信息，您可以快速响应，保证云手机流畅运行。
...
3. 执行以下命令，分别查询主机容器中进程的CPU及内存使用率等情况。如下图所示。
    ...
    若视频流云手机某些进程内存使用率高于85%，CPU使用率持续高于90%，则需要进一步排查该进程是否异常或请提交ISSUE反馈。
    ...
    若内存使用率高于85%，CPU使用率持续高于90%，请提交ISSUE反馈。
...
4. 执行以下命令，分别查询Docker容器中进程的CPU及内存占用率等情况。如下图所示。
    ...
    若视频流云手机某些进程内存占用率高于85%，CPU占用率持续高于90%，则需要进一步排查该进程是否异常或请提交ISSUE反馈。
    ...
    > ![](public_sys-resources/icon-note.gif)说明
    >
    >**ctop**是Docker管理的资源看管/记录工具，若未安装ctop，请先下载[ctop-0.7.5-linux-arm64](https://github.com/bcicen/ctop/releases/download/v0.7.5/ctop-0.7.5-linux-arm64)，将下载后的二进制重命名为ctop，并拷贝到"/usr/bin"目录下即可完成安装。
    >**ctop**命令回显参数说明：
    >- NAME：容器名称。
    >- CID：容器ID。
    >- CPU：容器CPU使用率。
...
### 看管/记录磁盘使用率状态<a name="ZH-CN_TOPIC_0000002518185538"></a>
...
3. 执行以下命令，查看视频流云手机服务器上的磁盘空间使用率。
    ...
    上图中视频流云手机容器数据存储在"/home/mount/data/"下，红色矩形圈出来的是容器android_1的数据存储信息，其存储大小为“16GB”，当前使用率为“4%”。若“Use%”的值均不大于85%，则磁盘空间使用率正常。否则，需要处理磁盘空间。

### 看管/记录负载<a name="ZH-CN_TOPIC_0000002518345502"></a>
