...
>
> 应用兼容性测试条件说明：<br>1.测试环境应为2280服务器的标准硬件环境<br>2.测试云手机规格(8核1路/16g内存/64g存储)<br>3.兼容性测试场景，单个容器在测试期间只运行一款游戏，不同时运行多款游戏<br>4.当前版本暂不支持WIFI设置和WIFI搜索等操作；<br>5.当前版本不支持蓝牙设备；<br> 
>
...
|解决计划|初步确认为第三方网络问题，复现后修复CI机器端口问题即可解决。|
