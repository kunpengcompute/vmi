/*
 * 功能说明: 适配QUADRA硬件视频解码器，包括解码器初始化、启动、解码、停止、销毁等
 */
#ifndef VIDEO_DECODER_QUADRA_H
#define VIDEO_DECODER_QUADRA_H
...
public:
    VideoDecoderQuadra() = default;
    ~VideoDecoderQuadra() override;
...
private:
     ...
     *          false 失败
     */
    bool LoadQuadraSharedLib() const;
    ...
    /**
     * @功能描述: 预处理解码前的数据包，将数据写入quadra
     * @参数 [in] src 输入码流数据
     * @参数 [in] inputSize 输入数据大小
     * @返回值: sendSize 写入quadra数据大小
     *          NI_RETCODE_FAILURE(-1) 失败
     */
    ...
    /**
     * @功能描述: 处理从quadra读取的解码后的数据，将数据传给上层
     * @参数 [in] buffer 输出数据缓存
     * @参数 [in] maxLen 输出缓冲区最大长度(Byte)
    ...
    * 	   	    保证执行ni_device_dec_session_flush后可以继续正常解码
    ...
    */
    ...
    /**
    * @功能描述：数据预处理，将QUADRA 128位对齐数据转化为 32位对齐
    * @参数 [in/out] inputDataArr[]：待处理yuv数据
    */
    void DecoderPreHandleData(uint8_t *inputDataArr[]);

    void DecSessionSaveHdrs(uint8_t streamHeaders[], int headerSize);
    
    ni_codec_t m_codec = EN_H264;
    ni_xcoder_params_t m_decApiParams {};
    ni_session_context_t *m_sessionCtx = nullptr;
    ...
    bool m_mallocFlag = false;
    uint8_t *m_inputData = nullptr;
